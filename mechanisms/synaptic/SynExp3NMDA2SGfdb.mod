...
NEURON {
	...
	RANGE inf, tau1, tau2
        RANGE Ginc, Gmax, tau_G1, tau_G2, tau_Gact
	THREADSAFE
}
...
NET_RECEIVE(weight, g_unit (uS), wf, f, d1, t0 (ms), G1, G2, Gact, Ginact, mg0) { LOCAL Gd, Gm, Gc, mg
        INITIAL {
            mg0 = Mgblock(v)
            d1 = 1
            f  = 1
        ...
        :printf("at t %g: before: v = %g Ginc = %g G1 = %g G2 = %g Gact = %g Ginact = %g\n", t, v, Ginc, G1, G2, Gact, Ginact)

        Gd = exp(-(t-t0)/tau_Gact)
        Gact = Gact + Ginact*(1 - Gd)
        Ginact = Ginact*Gd

        Gc = exp(-(t-t0)/tau_G2*(mg + mg0)/2)
        Ginact = Ginact + G2*(1 - Gc)
        G1 = G1*exp(-(t-t0)/tau_G1)
        G2 = G2*Gc

        Gm = Ginc*Gact*Gfactor
        if (Gm > Gact) { Gm = Gact }
        G1 = G1 + Gm
        G2 = G2 + Gm
        Gact = Gact - Gm
        ...
        wf = weight*g_unit*mg*factor*d1*f*(G2 - G1)
        :printf("at t %g: v = %g Mgblock(v) = %g G2-G1 = %g wf = %g\n", t, v, mg, G2-G1, wf)

        :printf("at t %g: v = %g Ginc = %g m = %g d1 = %g f = %g G1 = %g G2 = %g wf = %g Gact = %g Ginact = %g, Gsum = %g\n", t, v, Ginc, Gm, d1, f, G1, G2, wf, Gact, Ginact, Gsum)
